...
public:
   ...
   */
  template <typename Comp> void add_component(std::shared_ptr<Comp> component) {
   ...
   */
  template <typename Comp> void remove_component() {
   ...
   * otherwise.
   */
  template <typename Comp> bool contains() { return components.count(typeid(Comp).name()); }
   ...
   */
  template <typename Comp> std::shared_ptr<Comp> get_component() {
    auto it = components.find(typeid(Comp).name());
...
public:
   ...
   * @return std::vector<std::shared_ptr<Entity>> - Vector of shared pointers to entities with the given component
   * type
   */
  template <typename Comp> std::vector<std::shared_ptr<Entity>> get_entities_with_components() {
